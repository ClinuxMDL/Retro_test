...
def main(N = 15, folder = ''):
	...
	total_nonreaction = 0
	end_ix= start_ix+100000 if (start_ix + 100000)<610000 else -1
	alllines = open("../../train_all.txt").readlines()[start_ix:end_ix]
	N = len(alllines)+1
	example_doc={}
	outputfile=open("record_rr{}.txt".format(start_ix),"w")
	...
	try: # to allow breaking
		...
		for line in alllines:
			...
			example_doc[reaction_smiles_field]= line.split(",")[1]
			example_doc['_id']=line.split(",")[0]
			i += 1
			...
			try:
				...
				if correct or DO_NOT_TEST:	
					...
					rxn_cano_list.append(rxn_canonical)
					print("{},{},{}".format(example_doc['_id'],rxn_canonical,retro_canonical), file=outputfile)
	...
	print("The raw rxn smarts: {}; The unique rxn smarts: {}".format(len(rxn_cano_list), len(set(rxn_cano_list))))
...
if __name__ == '__main__':
	...
	parser.add_argument('-t', '--test', type = str, default = '0',
	...
	parser.add_argument('-ix', '--start_ix', type=int, default=100000,
						help='Maximum number of records to examine; defaults to 50')
